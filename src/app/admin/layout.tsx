          ...
          <nav className="flex flex-wrap gap-x-5 gap-y-1">
            {LINKS.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="flex min-h-11 items-center font-display text-xs font-bold tracking-[0.08em] text-ink-600 uppercase underline-offset-4 hover:text-azur-700 hover:underline"
              >
                {link.label}
              </Link>
            ))}
