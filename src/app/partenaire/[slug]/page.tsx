      ...
      <section className="bg-cross-700 text-enamel-50">
        <div className="mx-auto w-full max-w-5xl px-5 py-8 sm:px-8 sm:py-12">
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            <span className="border border-cross-400/50 px-2 py-1 font-display text-[11px] font-bold tracking-[0.1em] uppercase">
              {partner.category.name}
            </span>
            {partner.verificationStatus === "VERIFIED" ? (
              <span className="bg-cross-500 px-2 py-1 font-display text-[11px] font-bold tracking-[0.1em] text-cross-950 uppercase">
                Profil vérifié
              </span>
            ) : (
          ...
          <p className="mt-2 text-lg text-cross-100">
            ...
            {partner.category.supportsAppointments ? (
              <>
                ...
                <Link
                  ...
                  className="mt-4 block bg-cross-500 px-4 py-3 text-center font-display text-sm font-bold tracking-[0.06em] text-cross-950 uppercase hover:bg-cross-400"
                >
                  Prendre rendez-vous
                </Link>
              </>
...
function OpenNow({ state }: { state: OpenState }) {
  if (state.status === "open") {
    return (
      <span className="inline-flex items-center gap-2 bg-cross-500 px-3 py-1.5 font-display text-sm font-bold text-cross-950">
        <span aria-hidden className="h-2 w-2 bg-cross-950" />
  ...
  if (state.status === "closed") {
    return (
      <span className="inline-flex items-center gap-2 border border-cross-200/50 px-3 py-1.5 font-display text-sm text-cross-100">
