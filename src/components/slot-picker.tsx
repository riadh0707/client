      ...
      {free.length > 0 && (
        <section className="bg-enamel-50 p-4 sm:p-5">
          ...
          <div
            ...
            className={
              chosen
                ? "border-2 border-azur-700 bg-white px-4 py-3"
          ...
          return (
            <li key={slot.startAt}>
              <button
                ...
                className={
                  isSelected
                    ? "flex min-h-11 w-full items-center justify-center border-2 border-azur-700 bg-azur-500 px-2 py-2.5 font-display text-sm font-bold tabular-nums text-azur-950"
                    : "flex min-h-11 w-full items-center justify-center border border-enamel-300 bg-white px-2 py-2.5 font-display text-sm tabular-nums text-ink-900 hover:border-azur-600 hover:text-azur-700"
                }
              >
                {slot.label}
              </button>
...
function SubmitButton({ disabled }: { disabled: boolean }) {
  ...
  return (
    <button
      ...
      className="mt-5 min-h-11 w-full bg-azur-500 px-4 py-3.5 font-display text-sm font-bold tracking-[0.06em] text-azur-950 uppercase hover:bg-azur-400 disabled:cursor-not-allowed disabled:bg-enamel-300 disabled:text-ink-400"
    >
      {pending
        ? "Envoi en cours…"
        : disabled
